...
hdu = fits.open('3C279_filtered_srcProb.fits')
mask = hdu[1].data['_2FGLJ1256.1-0547'] > 0.85
...
#E_txt = [ax.text(rain_drops['position'][i,0], rain_drops['position'][i,1],Es,color=rain_drops['color'][0]) 
...
def update(frame_number):
    ...
    TIMEMJD = (TIMEMET[0] + (frame_number + 1) * WIDTH)/86400. + MJDREFI + MJDREFF
    t = Time(TIMEMJD,format='mjd')
    txt_time.set_text(t.datetime.strftime("%d %b %Y %H:%M"))

    # Make all colors more transparent as time progresses.
    rain_drops['color'][:, 3][mask] -= 1.0/50
    rain_drops['color'][:,3][mask] = np.clip(rain_drops['color'][:,3][mask], 0, 1)

    #Fade out the energy texts
    #[E_txt[i].set_color(color) for i,color in enumerate(rain_drops['color'][:15])]
    ...
    rain_drops['color'][:,3][new] = np.clip(rain_drops['color'][:,3][new], 0, 1)

    # Update the histogram with a new frame
    hist = np.histogram2d(rain_drops['position'][mask][:,0], rain_drops['position'][mask][:,1])
    im.set_data(hist[0])    
...
# director.
animation = FuncAnimation(fig, update, frames=FRAMES, interval=50, blit=False, repeat=False)
#animation.save('3C279_scale.gif', writer='imagemagick', fps=30,savefig_kwargs={'facecolor':'black'})
animation.save('3C279_full.mp4', fps=30, extra_args=['-vcodec', 'libx264', '-pix_fmt', 'yuv420p'],savefig_kwargs={'facecolor':'black'})
plt.show()
